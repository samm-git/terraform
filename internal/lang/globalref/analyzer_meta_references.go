...
func (a *Analyzer) MetaReferences(moduleAddr addrs.ModuleInstance, ref *addrs.Reference) (addrs.ModuleInstance, []*addrs.Reference) {
	...
	case addrs.Resource:
		return a.metaReferencesResource(moduleAddr, targetAddr, ref.Remaining)
	case addrs.ResourceInstance:
		return a.metaReferencesResource(moduleAddr, targetAddr.Resource, ref.Remaining)
	default:
...
func (a *Analyzer) metaReferencesResource(moduleAddr addrs.ModuleInstance, addr addrs.Resource, remain hcl.Traversal) (addrs.ModuleInstance, []*addrs.Reference) {
	modCfg := a.ModuleConfig(moduleAddr)
	if modCfg == nil {
		return moduleAddr, nil
	}

	rc := modCfg.ResourceByAddr(addr)
	if rc == nil {
		return moduleAddr, nil
	}
	...
	resourceTypeSchema := providerSchema.ResourceTypes[addr.Type]
